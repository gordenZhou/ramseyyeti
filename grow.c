...
void outputtofile(char* ostr,FILE* logfp){
    fprintf(logfp,"%s",ostr);
}
/*
...
int main(int argc,char *argv[])
{
    ...
    edge_list = FIFOInitEdge(TABOOSIZE);
    if(edge_list == NULL) {
		exit(1);
	}
	
	char logfileName[120];
	sprintf(logfileName,"../log/growlog_%s.txt",argv[1]);
	char datafileName[120];
	sprintf(datafileName,"../data/growr66_%s.txt",argv[1]);
...
//		FIFOInsertEdgeCount(taboo_list,best_i,best_j,count);
        ...
        /*if (ini > 0 && best_count > 200){
            outputtofile("Reset 30 Edges\n",logfp);
		...
		if ((ffct >= gsize * maxlin(6,gsize/10) && psen >= gsize*3)){
